...
/**
 * A service for Meowcoincore to enable HTTP routes to query information about the blockchain.
 *
...
var InsightAPI = function(options) {
  ...
  this.coinTicker = options.coinTicker || 'https://api.coinmarketcap.com/v1/ticker/meowcoin/?convert=USD';
...
InsightAPI.dependencies = ['meowcoind', 'web'];
...
InsightAPI.prototype.start = function(callback) {
  ...
  this.node.services.meowcoind.on('tx', this.transactionEventHandler.bind(this));
  this.node.services.meowcoind.on('block', this.blockEventHandler.bind(this));
...
InsightAPI.prototype.setupRoutes = function(app) {
  ...
  // Asset routes
  var assets = new AssetController({node: this.node});

  app.get('/assets', this.cacheShort(), assets.listAssets.bind(assets));
  app.get('/assets/restricted', this.cacheShort(), assets.listRestrictedAssets.bind(assets));
  app.get('/assets/tags', this.cacheShort(), assets.listQualifierAssets.bind(assets));
  app.get('/assets/qualifiers', this.cacheShort(), assets.listQualifierAssets.bind(assets));
  app.get('/assets/frozen', this.cacheShort(), assets.listGlobalFrozen.bind(assets));
  app.get('/asset/:asset/verifier', this.cacheShort(), assets.getVerifierString.bind(assets));
  app.get('/asset/:asset/checkFrozen', this.cacheShort(), assets.checkFrozen.bind(assets));
  app.get('/asset/:asset/tagged', this.cacheShort(), assets.listAddressesForTag.bind(assets));
  app.get('/assets/checkVerifier', assets.checkVerifier.bind(assets));
  app.post('/assets/checkVerifier', assets.checkVerifier.bind(assets));
  ...
  app.get('/addr/:addr/asset/:asset/utxo', this.cacheShort(), addresses.checkAddr.bind(addresses), addresses.utxo.bind(addresses));
  app.get('/addrs/:addrs/asset/:asset/utxo', this.cacheShort(), addresses.checkAddrs.bind(addresses), addresses.multiutxo.bind(addresses));
  ...
  app.get('/addr/:addr/tags', this.cacheShort(), addresses.checkAddr.bind(addresses), addresses.tagsForAddress.bind(addresses));
  app.get('/addr/:addr/frozen', this.cacheShort(), addresses.checkAddr.bind(addresses), addresses.frozenForAddress.bind(addresses));
  app.get('/addr/:addr/checkFrozen/asset/:asset', this.cacheShort(), addresses.checkAddr.bind(addresses), addresses.checkFrozen.bind(addresses));
  app.get('/addr/:addr/checkTag/asset/:asset', this.cacheShort(), addresses.checkAddr.bind(addresses), addresses.checkTag.bind(addresses));

  app.get('/addrs/:addrs/balance', this.cacheShort(), addresses.checkAddrs.bind(addresses), addresses.balancesum.bind(addresses));

  // Address property routes

  app.get('/addr/:addr/balances', this.cacheShort(), addresses.checkAddr.bind(addresses), addresses.balances.bind(addresses));
...
InsightAPI.prototype.getPublishEvents = function() {
  return [
    ...
    },
    {
        ...
        scope: this,
        subscribe: function (emitter) {
            this.subscribe(emitter, 'meowcoin');
        }.bind(this),
        unsubscribe: function (emitter) {
            this.unsubscribe(emitter, 'meowcoin');
        },
        extraEvents: ['meowcoin/tx', 'meowcoin/block']
    }
  ];
};
...
InsightAPI.prototype.blockEventHandler = function(hashBuffer) {
  ...
  if (self.subscriptions.meowcoin.length) {
      ...
      this.blocksController.getBlockByHash(hashBuffer.toString('hex'), function (err, block) {
          ...
          async.mapSeries(block.tx, function(txid, next) {
              ...
              // Notify meowcoin subscribers
              for (var i = 0; i < self.subscriptions.meowcoin.length; i++) {
                  self.subscriptions.meowcoin[i].emit('meowcoin/block', {
                      block: block,
...
InsightAPI.prototype.transactionEventHandler = function(txBuffer) {
   ...
   if (this.subscriptions.inv.length || this.subscriptions.meowcoin.length) {
       ...
       if (this.subscriptions.meowcoin.length) {
           var transformedTrx = this.txController.transformMeowcoinTransaction(tx);

           for (var i = 0; i < this.subscriptions.meowcoin.length; i++) {
               this.subscriptions.meowcoin[i].emit('meowcoin/tx', transformedTrx);
           }
