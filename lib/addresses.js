...
AddressController.prototype.balances = function(req, res) {
  this.addressSummarySubQuery(req, res, 'balances');
}
...
AddressController.prototype.check = function(req, res, next, addresses) {
  ...
  for(var i = 0; i < addresses.length; i++) {
    try {
      var a = new meowcoincore.Address(addresses[i]);
    } catch(e) {
      return self.common.handleErrors({
...
AddressController.prototype.utxoWithoutMempool = function(req, res) {
  ...
  var opts = { queryMempool: false };
  if (req.params.asset) {
    opts.assetName = req.params.asset;
  }

    this.node.getAddressUnspentOutputs(req.addrs, opts, function(err, utxos) {
        if(err && err.code === -5) {
           return res.jsonp([]);
        } else if(err) {
            return self.common.handleErrors(err, res);
...
AddressController.prototype.utxo = function(req, res) {
  ...
  this.node.getAddressUnspentOutputs(req.addr, opts, function(err, utxos) {
    if(err) {
      return self.common.handleErrors(err, res);
...
AddressController.prototype.tagsForAddress = function(req, res) {
  var self = this;

  var address = req.params.addr || req.query.addr || req.body.addr || '';
  if(address.length == 0) {
    return self.common.handleErrors({
      message: 'Must include an address.',
      code: 1
    }, res);
  };  

  this.node.listTagsForAddress(address, function(err, data) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
...
AddressController.prototype.checkTag = function(req, res) {
  var self = this;

  var address = req.params.addr || req.query.addr || req.body.addr || '';
  var asset = req.params.asset || req.query.asset || req.body.asset || '';
  if(address.length == 0 || asset.length == 0) {
    return self.common.handleErrors({
      message: 'Must include an address and a restricted asset.',
      code: 1
    }, res);
  };  
...
AddressController.prototype.frozenForAddress = function(req, res) {
  var self = this;
  ...
  this.node.listAddressFrozen(address, function(err, data) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
...
AddressController.prototype.checkFrozen = function(req, res) {
  var self = this;
