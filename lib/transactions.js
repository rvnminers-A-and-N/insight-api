...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  if(transaction.height >= 0) {
    confirmations = this.node.services.meowcoind.height - transaction.height + 1;
  }
...
TxController.prototype.transformOutput = function(options, output, index) {
  ...
  if (output.address) {
    ...
    var address = meowcoincore.Address(output.address); //TODO return type from meowcoincore-node
    transformed.scriptPubKey.type = address.type;
  }

  if (output.asset) {
    transformed.assetType = output.assetType;
    transformed.asset = output.asset;
  }
...
TxController.prototype.transformMeowcoinTransaction = function(transaction) {
