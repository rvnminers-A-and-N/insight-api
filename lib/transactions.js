...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  if(transaction.height >= 0) {
    confirmations = this.node.services.ravend.height - transaction.height + 1;
  }
...
TxController.prototype.transformOutput = function(options, output, index) {
  ...
  if (output.address) {
    ...
    var address = ravencore.Address(output.address); //TODO return type from ravencore-node
...
TxController.prototype.transformRavenTransaction = function(transaction) {
