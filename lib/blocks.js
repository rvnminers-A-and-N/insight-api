...
BlockController.prototype.getBlockByHash = function(hash, next) {
    ...
    if (blockCached) {
        ...
        blockCached.confirmations = self.node.services.meowcoind.height - blockCached.height + 1;
        ...
        return async.waterfall([function (callback) {
            ...
            return self.node.services.meowcoind.getBlockHeader(hash, function (err, info) {
...
BlockController.prototype._normalizePrevHash = function(hash) {
  // TODO fix meowcoincore to give back null instead of null hash
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  this.node.services.meowcoind.getBlockHeader(parseInt(height), function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
      ...
      return async.setImmediate(function() {
        ...
        return async.waterfall([function (callback) {
            return self.node.services.meowcoind.client.getBlock(hash, function (err, response) {
      ...
      if (err) {
        ...
        var confirmations = self.node.services.meowcoind.height - block.height + 1;
...
BlockController.prototype.list = function(req, res) {
  ...
  var moreTimestamp = lte;
    ...
    return self.node.services.meowcoind.getBlockHashesByTimestamp(lte, gte, function (err, hashes) {
